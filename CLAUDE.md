...
- **Mobile** (`mobile/app/team/[name].tsx`): section renamed **"Pick Insight"**. Card now shows Pick % / Avg Win Prob / Accuracy (with the raw W-L record demoted to a small subtext under Accuracy, not the headline anymore), two comparison bars ("Picked by Gridirons" vs "Model's Win Probability"), and a conditional insight callout — only shown when a gap is actually notable (≥12 pts, `INSIGHT_THRESHOLD` in `team/[name].tsx`), picking whichever of two independent gaps is larger:
  - Pick % vs. Avg Win Prob → **"Fan favorite"** (picked more than the model justifies) or **"Slept on"** (model likes them more than the Gridirons do)
  - Accuracy vs. Avg Win Prob → **"Beats the odds"** (wins more than the model expects) or **"Overrated by the model"** (loses more than expected) — echoes the existing per-user `upset_pick`/`contrarian` Achievement concepts, just aggregated per-team/season instead of per-user/week
  - **Deliberately NOT changed**: Accuracy stays pick-weighted (a game 9 people picked counts 9x toward it, same population as Pick %) rather than switching to a game-weighted team win rate — discussed with Nick, and pick-weighted is intentional (it's answering "of the times *we* called this team, were we right," not "what's their record," which is already shown in the hero section above). Known, accepted wrinkle: this means the "beats the odds"/"overrated" callout compares a pick-weighted number (Accuracy) against a game-weighted one (Avg Win Prob, see fix below) — a useful heuristic, not a statistically rigorous test, and noisy with very few games played (Accuracy is just 100%/0% off a single game). Left as-is, not revisited.
  - `team-central.tsx`'s separate compact per-row `pickAccuracy` display was NOT touched — different, simpler use case, no "Community Picks" naming to update there.
- **Bug found and fixed same session**: `avgWinProb` was originally computed by averaging each individual **pick's** win-probability snapshot (captured at submission time), not one number per game. Since different Gridirons pick at different times over the week — and the model's number can keep moving right up to kickoff — the same game's win prob got diluted by however many people picked it and whenever they happened to pick. Caught because Nick noticed the Seahawks showing 51% when he remembered ~63% from Game Detail; traced it via the actual pick rows — 9 Seahawks pickers had captured 5×61%/4×39% snapshots across Sept 3–8 as the model's number moved, averaging to 51%, while the real final line was 61%. Fixed in both `teams.ts` endpoints: `avgWinProb` now averages `winningTeamWinProb`/`losingTeamWinProb` (frozen once at game-final, see `espnService.ts`) **once per decided game**, not once per pick — confirmed live post-deploy (Seahawks now correctly show 61%).
- **Also fixed while in here**: the live "game went final → refresh stats" invalidation effect in `picks.tsx` only ever invalidated the team **list** query (`['teams']`) — Team Detail's own query (`['team']`, singular, used by `useTeamDetail`) was never covered, so this whole screen (Pick Insight included) could sit stale for up to 5 minutes (its `staleTime`) after a game finished. Now invalidated alongside `['teams']`/`['picks-by-team']`.
- Deployed Sept 10 2026: backend build + commit `df36bec` (redesign) + commit `f41117e` (avgWinProb fix), both pushed to `main` (Railway auto-deployed, avgWinProb fix confirmed live against production); mobile via `eas update --branch preview` (update group `0f811498-7865-4936-b68d-f94c306edaa1`, redesign only — the avgWinProb fix was backend-only, no OTA needed).

### ✅ Fixed "current week" defaulting to Week 2 during real Week 1 (admin, public profile, week-selector outline) — DONE (Sept 10 2026)
Nick noticed the public profile's pick comparison, Admin → NFL Tools' week picker, and the Picks/Week Picks screens' week-selector all pointed at/highlighted Week 2 while still actually in Week 1. Root cause: `getCurrentNFLWeek()` (`mobile/src/lib/nflSeason.ts`) is a hardcoded calendar-math guess — it assumes the season always starts "the first Thursday of September" (computes to Sept 3, 2026), then just counts 7-day blocks forward. But 2026's actual Week 1 didn't kick off until **Wednesday Sept 9** (a 6-day-later start than the formula assumes), so by Sept 10 the naive math had already rolled into "week 2." Three call sites were still reading this stale value directly instead of the server's data-driven `useCurrentWeek()`/`getCurrentWeekAndType()`:
1. `admin.tsx`'s `ToolsTab`/`DataTab`/`ActivityTab` — never wired to the data-driven hook at all (this was actually flagged as a known gap back in the Aug 10 2026 entry further down and never closed until now). Now each corrects its `week`/`seasonType` `useState` default once from `useCurrentWeek()`, same pattern `picks.tsx` already used.
2. `user/[id].tsx`'s `PickComparison` — same gap, same fix; also fixed the downstream effects that were silently relying on the same stale value (`maxWeek`, `isPastWeek`/`canShow` visibility gating, not just the initial default).
3. `picks.tsx`/`week-picks.tsx` — these two already self-corrected their actual displayed/selected week correctly via `useCurrentWeek()`, but `WeekSelector`'s separate blue "current week" **outline** prop was still fed the stale constant directly, so the selector showed the right week's data while outlining the wrong bubble. Both now pass a server-corrected `displayCurrentWeek` to that prop instead.
- Deployed Sept 10 2026: mobile-only (JS, no backend change) via `eas update --branch preview` (update group `96cd11b7-15e9-4401-a93c-d05b92494745`), commit `b26ff50`.

### ✅ Jacob h (Purdy Mouths) 2025 migration — DONE (Sept 10 2026)
Jacob signed up fresh (`jhayhurst714@gmail.com`, team name **"jacob h"**, Sept 8 2026) — confirmed he's the same player as 2025's "The Purdy Mouths." Ran the standard `npm run reassign:user -- --old MrwsvsZKPwTJ4SwYnAKtyPEAsrq2 --email jhayhurst714@gmail.com` from `server/`: migrated 272 picks + 23 trophies from the old 2025-migrated record to his new account, old record deleted. **Nick's call: left his team name as "jacob h"**, not renamed to match 2025 — the reassign script only ever moves data, never touches `teamName`, so this was a deliberate choice, not a leftover TODO. All 7 Gridirons are now fully migrated for 2026 (see the updated table below) — nothing left in the reassignment backlog.

### ✅ Admin tool to set a missed pick for a specific player — DONE (Sept 9 2026)
One of the Gridirons only made 4 of 16 Week 1 picks before lock — real money on the line (weekly bonus pool). Considered using "Unlock Week" to let him finish, but that's the wrong tool: the lock flag is global and gates both pick *visibility* and pick *editability* for the entire week at once, so reopening it would've re-hidden everyone's already-revealed Week 1 picks from each other AND reopened editing on every game for every player — including any of that Sunday's games already live or final, since the server only checks "is the week locked," never "has this specific game started." Built a properly scoped tool instead:
- `POST /api/admin/picks` (`server/src/routes/admin.ts`) — sets/overwrites one user's pick on one specific game directly, bypassing the week lock (that's the point). Safety rail: rejects any game that isn't `status: 'pre'` (hasn't kicked off yet) — this can never be used to retroactively set a pick once the outcome is knowable or even partially knowable (live). Always logged to `pick_audit_log` as `admin_edit`, same as every other pick action, so it shows up in the existing Activity-tab dispute trail.
- `GET /api/admin/picks` now also filters by `seasonType` (was missing it — same preseason/regular-season week-number collision bug that's been fixed elsewhere in this codebase; harmless before now since nothing called this route with a `week` param during an active preseason/regular overlap, but worth having fixed for this feature).
- Mobile: Admin → NFL Tools → **"Set Missing Pick"** section — pick a player from the chip row, then tap the correct team per game to set it. Already-picked games highlight green; games already past kickoff show disabled/"locked out" so the tool can't be misused on a live or finished game.
- **Also confirmed while investigating, worth remembering for future disputes**: a missing pick does NOT currently count as a loss. The leaderboard only counts picks that actually exist and are graded (`COUNT(CASE WHEN is_correct = ...)`), so a skipped game is simply excluded from that player's win/loss denominator, not scored against him. Doing nothing about a missed pick isn't a penalty — it just means fewer graded games for that player than everyone else.
- Deployed Sept 9 2026: backend build + commit `ef10b6f` + push (Railway auto-deploying); mobile via `eas update --branch preview` (update group `ee3fc369-5395-4fe9-8e25-9bd58a4296f3`).
...
- **✅ VERIFIED WORKING (Nick, Aug 30 2026)** — the dynamic lock-time fix is confirmed working end-to-end (preseason Week 4 locked on its real computed day with the reminder push, picks-locked push, default picks, and proof-of-picks email all firing correctly). No longer a pending verification item.

### ⚠️ BACKGROUND, NOT SEASON-BLOCKING — iOS App Store rejected 3× (latest Aug 30 2026), confirmed to be the in-app team logos (season plan is TestFlight Internal Testing — see decision above)
...
**Rejected a 3rd time Aug 30 2026 — Apple confirmed it's the logos.** Same generic 4.1(a) "metadata appears to contain potentially misleading references to third-party content" message, but this time Apple **attached the specific offending image: Nick's screenshot of a game box score showing team logos.** So the diagnosis below is now confirmed, not a guess — the logos in the screenshots are what's blocking it. **Nick's call (Aug 30 2026): to get on the public App Store, the logos have to go.** He's not happy about it ("it looks so nice") — see the discussion note right below this entry for the current thinking on paths forward. Not started; not season-blocking (TestFlight Internal Testing carries the season regardless).

**Rejected Aug 25 2026, two separate issues:**
1. **Guideline 4.1(a) — the screenshots** (confirmed Aug 30, see above). Every screenshot in `Screenshots/iOS/` (7 images) shows real NFL team logos, since they're captures of the real app showing real games, and screenshots count as "metadata" for App Store review.
   - **There is no way to get a clean screenshot without changing the app itself first** — you can't screenshot around real logos when every game shown is a real game. The fix is replacing ESPN's real logo artwork with generic team-colored badges (color + abbreviation, no logo shape) app-wide, *then* retaking screenshots against that. Team names as plain text are believed fine to leave alone (well-settled fair use across the whole fantasy-sports category, and Apple's language reads as being about the recognizable graphic marks specifically) — only the logo images need replacing.
...
**Where this stands after the 3rd rejection (Aug 30 2026):**
- **Public App Store is NOT needed this season.** TestFlight Internal Testing (100-tester cap, no guideline review) covers all 7 Gridirons for the whole 2026 season — that was the Aug 25 decision and it still holds. So the logo removal is purely for an eventual public listing, no deadline.
- **When Nick does want the public listing**, the logo work is unavoidable — Apple has now explicitly pointed at a logo screenshot, and reviewers can (and clearly do) open the running app, so a screenshots-only workaround won't survive review. Using ESPN's logo artwork with no NFL license is also genuine legal exposure, not just an Apple-pickiness problem — the "every competitor does it" argument fails because ESPN/Yahoo/Sleeper/DK all have license deals.
- **The badge design doesn't have to look bad.** A well-made color + wordmark/abbreviation badge system (think The Athletic, or the cleaner betting apps) can look sharp and consistent. Before committing to the full swap, do a **design mockup pass** (a few `TeamBadge` visual options against real GameCard/Game Detail layouts) so Nick can judge whether it's actually a downgrade. Only proceed to the app-wide swap if he's OK with a chosen style.
- Draft plan for the eventual swap is in item 1 above + the "logo/screenshot" notes further down. Pure UI + OTA, no backend/DB change (keep syncing logo URLs, just stop rendering them).
...
7. **Team badge / color system — mock-up pass, THEN app-wide logo swap (for the future public App Store listing)** — Nick's request Aug 30 2026, after the 3rd iOS rejection confirmed the in-app NFL logos are what's blocking a public listing (see the "rejected 3×" entry near the top of this doc for full context). **Not needed for the season — TestFlight covers all 7 Gridirons.** Do this whenever Nick decides he wants a public listing.
   - **Step 1 — design mock-up (do this first, get Nick's sign-off before any app-wide change):** produce a few `TeamBadge` visual options rendered against the *real* GameCard and Game Detail layouts so Nick can judge whether badges are an acceptable substitute for the logos. Directions discussed: (a) solid team-primary color disc + white abbreviation ("PHI"); (b) split two-tone (primary/secondary diagonal) + abbreviation, closest to a logo's visual weight; (c) wordmark chip — rounded rect in team color with city/nickname as text. Pick one style with Nick.
   - **Step 2 — build:** new static `mobile/src/lib/teamColors.ts` (32-team map: full name → abbreviation + primary/secondary hex — the app currently stores ZERO team color/abbrev data, only ESPN's raw logo URLs in `games.homeTeamLogo`/`awayTeamLogo`), new `TeamBadge` component, swap it in for every real-logo `<Image>` across `GameCard.tsx`, `game/[id].tsx`, `week-picks.tsx`, `team/[name].tsx`, `team-central.tsx`, `picks-by-team.tsx`, `user/[id].tsx`. Backend/DB untouched (keep syncing logo URLs, just stop rendering them). Pure UI, ships via OTA.
   - **Step 3 — retake all 7 iOS screenshots** against the badge build, re-upload in App Store Connect, resubmit. Also apply the trivial Sign in with Apple `buttonStyle` fix (BLACK → WHITE, see the rejection entry) in the same pass since it's the other open rejection reason.
...
| jacob h | jhayhurst714@gmail.com | no | The Purdy Mouths | ✅ done (reassign:user, Sept 10 2026) — signed up as "jacob h", Nick's call to leave the team name as-is rather than rename to match 2025 |
