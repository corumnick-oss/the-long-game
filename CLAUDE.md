...
- **First attempt didn't actually work on-device.** Nick confirmed a full force-close + reopen (saw the OTA download happen), but timestamps still showed hours off (e.g. an 11:59PM PT default-pick event displaying as ~6:59AM) — ruling out a stale-bundle explanation. Root cause: `toLocaleString`'s `timeZone` option isn't reliably honored by Hermes on-device — confirmed the underlying data and the exact same formatting code were correct when run directly in Node (full ICU support there), so the bug is specifically the on-device JS engine silently falling back to an unconverted/UTC render instead of erroring or honoring the option. This is a known class of RN/Hermes issue (incomplete ICU timezone data on-device) — **any future on-device PT-forced timestamp must use `mobile/src/lib/pacificTime.ts` (`formatPacific`/`formatPacificDateOnly`), never `toLocaleString(..., { timeZone: 'America/Los_Angeles' })` directly** — that new helper computes the offset manually from the standard US DST rule instead of depending on device ICU data. Server-side code (`emailService.ts`, `notificationService.ts`) is unaffected — Node has full ICU, confirmed by directly testing the same code there.
- **Redeployed** Aug 24 2026 via `eas update --branch preview` (update group `9fbfb3c3-8705-486e-837f-3f365253fab2`). Still needs Nick to force-close/reopen and reconfirm the Activity tab shows correct PT times now.
...
2. **Google Sign-In on Android — two issues found, only the first is confirmed fixed:**
   ...
   - **Aug 24 2026: Nick reports `DEVELOPER_ERROR` again** — not the silent-hang symptom from the bullet above, the *original* error code, which was supposedly fully resolved Aug 18 by registering both the Play Store App Signing cert and the EAS preview-keystore cert in Firebase. `DEVELOPER_ERROR` on Android Google Sign-In is essentially always an unregistered SHA-1 cert (or a webClientId/package-name mismatch — checked `mobile/.env`'s `EXPO_PUBLIC_GOOGLE_WEB_CLIENT_ID` against what's documented above, matches, so not that). Ruled out re-checking anything server-side without more info from Nick — **did not yet get an answer on**: (a) is this the same dad's-phone sideloaded internal/preview APK as before, or a different device/build (e.g. the Aug 16-built production AAB, which is signed with a THIRD, never-yet-registered keystore, sideloaded directly rather than through Play Store's re-signing — very plausible if this is what's now being tested), and (b) whether a new `eas build` for Android happened since Aug 18 that could have touched credentials. **Next session: get those answers first**, then if a new build/device is confirmed, have Nick run `eas credentials -p android` (interactive terminal only, same limitation as before) to find that build's SHA-1 and add it as a third fingerprint in Firebase Console alongside the other two — never replace, only add.
